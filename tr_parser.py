...
SUBJECT_RE = re.compile(r'^\d[A-Z]{2}\d{3}[A-Z0-9]{2,6}$')
    ...
    while i < n:
        ...
        if SUBJECT_RE.match(t):
            ...
            while i < n and not SUBJECT_RE.match(tokens[i]['text']):
                tx = tokens[i]['text']
                if NUM_RE.match(tx) or PLACEHOLDER_RE.match(tx):
                    # numeric mark or an absent/withheld placeholder occupying a mark slot
                    nums.append(tx)
                    i += 1
        ...
        for li, line in enumerate(rec_lines):
            ...
            if li == 0:
                ...
                if abs(offset - 11.1) <= 4:
                    ...
                    # mother's name (always present at this fixed slot)
                    rec['mother_name'] = ' '.join(texts)
...
def extract_header_meta(page):
    ...
    for w in words:
        if 140 <= w['top'] <= 160 and 895 <= w['x0'] <= 925 and NUM_RE.match(w['text']):
            meta['out_of'] = w['text']
            break
    return meta
...
def parse_pdf(path, progress_cb=None):
    ...
    with pdfplumber.open(path) as pdf:
        ...
        for pi, page in enumerate(pdf.pages):
            ...
            # never from inside a record's own mark data. ---
            ...
            if legend_words:
                ...
                if cur:
                    lines_txt.append(' '.join(cur))
                legend_text = ' '.join(lines_txt)
                for m in re.finditer(r'(\d[A-Z]{2}\d{3}[A-Z0-9]{2,6})\s+([^,]+?)(?:,|$)', legend_text):
                    code, cname = m.group(1), m.group(2).strip()
                    if code not in subject_legend and len(cname) > 2:
                        subject_legend[code] = cname
